...
router.get('/entryno', async (req, res) => {
    var lastData = await purchaseModel.findOne().sort({createdat: 'desc'})
...
router.get('/list', async (req, res) => {
    if(req.query.entryno){
            var list = await purchaseModel.findOne({ entryno: req.query.entryno })
            if (!list) {
        ...
        var list = await purchaseModel.find({});
        if (!list || !list.length) {
            res.status(403).json({ 'message': 'No data found' });
            return;
        } else {
...
router.post('/create', async (req, res) => {
    try {
        const data = req.body;
        var exists = await purchaseModel.findOne({ entryno: data.entryno });
        if (exists) {
            res.status(400).json({ 'message': 'User entryno already exists' });
            return;
        }
...
router.post('/update/:_id', async (req, res) => {
    ...
    try{
        ...
        var entryExists = await purchaseModel.findOne({ entryno: data.entryno });
        ...
        // `new: true`
        const doc = await purchaseModel.findOneAndUpdate(filter, data, {
            new: true
        });
